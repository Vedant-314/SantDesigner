...
export const CartProvider = ({ children }) => {
    ...
    useEffect(() => {
        const syncCartWithBackend = async () => {
            if (user) {
                try {
                    const response = await axios.post('http://localhost:5002/api/user/update-cart', {
                        userId: user._id,
                        cart: state.cart,
                    });
        ...
        if (JSON.stringify(state.cart) !== JSON.stringify(prevCartRef.current)) {
            syncCartWithBackend(); // Call sync function whenever the cart changes
            prevCartRef.current = state.cart; // Update the previous cart
        }
