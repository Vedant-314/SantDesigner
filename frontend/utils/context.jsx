...
const initialState = {
  cart: JSON.parse(localStorage.getItem("cart")) || [], // Get cart from local storage if available
  subtotal: 0,
};
...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    const syncCartWithBackend = async () => {
      if (user) {
        try {
          const response = await axios.post("/api/user/update-cart", {
            userId: user._id,
            cart: state.cart,
          });
          reduxDispatch(setUser({ ...user, cart: response.data }));
    ...
    // Sync the cart only if it has changed
    if (JSON.stringify(state.cart) !== JSON.stringify(prevCartRef.current)) {
      syncCartWithBackend();
      prevCartRef.current = state.cart;
    }

    // Save cart to local storage regardless of user status
    localStorage.setItem("cart", JSON.stringify(state.cart));
  }, [state.cart, user, reduxDispatch]);

  // Load user's cart from the backend or local storage
  useEffect(() => {
    if (user && user.cart) {
      // Load cart from the user object if logged in
      dispatch({ type: "SET_CART", payload: user.cart });
    } else {
